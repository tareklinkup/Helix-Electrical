...
class Employee extends CI_Controller
{
    ...
    public function shift()
    {
        ...
        $data['title'] = 'Shift';
        $data['content'] = $this->load->view('Administrator/employee/shift', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function insert_shift()
    {
        ...
        $query = $this->db->query("SELECT name from tbl_shifts where name = '$name'");
        ...
        if ($query->num_rows() > 0) {
            ...
            $data = array(
                "name" => $this->input->post('shift', TRUE)
            );
            if ($this->mt->save_data('tbl_shifts', $data)) {
                $message = "Shift insert success";
                echo json_encode($message);
            }
        }
    }
    ...
    {
        $query = $this->db->query("SELECT * from tbl_shifts where shift_id = '$id'");
        $data['row'] = $query->row();
        $this->load->view('Administrator/employee/edit_shift', $data);
    }
    ...
    public function updateShift()
    {
        ...
        if ($this->mt->update_data("tbl_shifts", $data, $id, $fld)) {
            redirect('shift');
        }
    }
