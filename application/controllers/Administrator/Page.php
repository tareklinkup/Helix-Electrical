...
class Page extends CI_Controller {
    ...
    public function getMaterialCategories(){
        $categories = $this->db->query("select * from tbl_materialcategory where status = 'a' and category_branchid = '$this->brunch'")->result();
        echo json_encode($categories);
    }

    public function add_material_category()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Add Category";
        $data['content'] = $this->load->view('Administrator/add_matcategory', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function insert_material_category()  {
		$catname = $this->input->post('catname');
		$brunch = $this->brunch;
        $query = $this->db->query("SELECT * from tbl_materialcategory where category_branchid = '$brunch' AND ProductCategory_Name = '$catname'");
        if($query->num_rows() > 0){
            $this->db->query("update tbl_materialcategory set status = 'a' where ProductCategory_SlNo = ?", $query->row()->ProductCategory_SlNo);
        }
        else{
            $data = array(
                "ProductCategory_Name"              => $this->input->post('catname', TRUE),
            ...
            $this->mt->save_data('tbl_materialcategory',$data);
			$success = 'Save Success';
            echo json_encode($success);    
        }
    }

    public function materialCatDelete(){
        $id = $this->input->post('deleted');
        $fld = 'ProductCategory_SlNo';
        $this->mt->delete_data("tbl_materialcategory", $id, $fld);
		$success = 'Delete Success';
        echo json_encode($success);
    } 

    public function matcatedit($id)  {
        $data['title'] = "Edit Category";
        //$fld = 'ProductCategory_SlNo';
        $data['selected'] = $this->Billing_model->select_by_id('tbl_materialcategory', $id,'ProductCategory_SlNo');
        $data['content'] = $this->load->view('Administrator/edit/material_category_edit', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function matcatupdate(){
        $id = $this->input->post('id');
        $catname = $this->input->post('catname');
		$brunch = $this->brunch;
        $query = $this->db->query("SELECT * from tbl_materialcategory where category_branchid = '$brunch' AND ProductCategory_Name = '$catname' and ProductCategory_SlNo != '$id'");
        if($query->num_rows() > 0){
            $this->db->query("update tbl_materialcategory set status = 'a' where ProductCategory_SlNo = ?", $query->row()->ProductCategory_SlNo);
        } else {
            ...
            if($this->mt->update_data("tbl_materialcategory", $data, $id,$fld)){
				$msg = true;
				echo json_encode($msg);
            }
        }
    ...
    public function addBranch()  {
        ...
        try{
            ...
            $newBranch = array(
                ...
                'is_production'     => $branch->is_production == true ? 'true' : 'false',
                'Brunch_sales'      => '2',
