...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        $res = ['success'=>false, 'message'=>'', 'productId' => $this->mt->generateProductCode()];
        try{
            ...
            if($productNameCount > 0){
                ...
                $res = ['success'=>false, 'message'=>'Product name already exists', 'productId' => $Product_Code];
            ...
            if($productCodeCount > 0){
                $Product_Code = $this->mt->generateProductCode();
                $res = ['success'=>false, 'message'=>'Product code already exists', 'productId' => $Product_Code];
                echo json_encode($res);
                exit;
            }
            ...
            $res = ['success'=>false, 'message'=>$ex->getMessage(), 'productId' => $this->mt->generateProductCode()];
        }
    ...
    public function updateProduct(){
        ...
        try{
            ...
            if($productNameCount > 0){
                $res = ['success'=>false, 'message'=>'Product name already exists', 'productId' => $productObj->Product_Code];
                echo json_encode($res);
                exit;
            }
            ...
            if($productCodeCount > 0){
                $res = ['success'=> false, 'message'=>'Product code already exists', 'productId' => $productObj->Product_Code];
                echo json_encode($res);
                exit;
            }
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pp.quantity), 0)
                    ...
                    and pp.status = 'a'
                    " . (isset($data->date) && $data->date != null ? " and pro.date <= '$data->date'" : "") . "
