...
class Reports extends CI_Controller {
    ...
    public function materialList()
	{
        ...
        ")->result();
		$data['content'] = $this->load->view('Administrator/reports/materialList', $data, true);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getBalanceSheet()
    {
        ...
        try {
            ...
            $stocks = $this->db->query("
                select
                    ...
                    (select ifnull(sum(pp.quantity), 0)
                        ...
                        join tbl_productions tp on tp.production_id = pp.production_id
                        where pp.product_id = p.Product_SlNo
                        and pp.branch_id = '$branchId'
                        and pp.status = 'a'
                        " . (isset($date) && $date != null ? " and tp.date < '$date'" : "") . "
            ...
            //material stocks
            $material_stocks = $this->db->query("
                select
                    ifnull(
                        (
                            select sum(mpd.quantity) 
                            from tbl_material_purchase_details mpd
                            join tbl_material_purchase mp on mp.purchase_id = mpd.purchase_id
                            where mpd.material_id = m.material_id 
                            and mpd.status = 'a'
                            " . (isset($date) && $date != null ? " and mp.purchase_date < '$date'" : "") . "
                        ), 0.00
                    ) as purchased_quantity,
                    ...
                    ifnull(
                        (
                            ...
                            and pd.status = 'a'
                            " . (isset($date) && $date != null ? " and pr.date < '$date'" : "") . "
                        ), 0.00
                    ) as production_quantity,

                    ifnull(
                        (
                            select sum(mdd.damage_quantity) 
                            from tbl_material_damage_details mdd
                            join tbl_material_damage md on md.damage_id = mdd.damage_id
                            where mdd.material_id = m.material_id 
                            and mdd.status = 'a'
                            " . (isset($date) && $date != null ? " and md.damage_date < '$date'" : "") . "
                        ), 0.00
                    ) as damage_quantity,
            ...
            $material_stock_value = array_sum(
                array_map(function($product){
                    return $product->stock_value;
                }, $material_stocks));
            ...
            $other_income_expense = $this->db->query("
                ...
                (
                    ...
                    and mdd.status = 'a'
                    " . ($date == null ? "" : " and md.damage_date  < '$date'") . "
            ...
            $net_profit = (
                ...
                $other_income_expense->material_damaged_amount + 
            ...
            $statements = [
                ...
                'material_stock_value'  => $material_stock_value,
