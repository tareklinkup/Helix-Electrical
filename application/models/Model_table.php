...
class Model_Table extends CI_Model{
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset + paid_material_purchase
            ) as total_out,
            (
    ...
    public function supplierDue($clauses = "" , $date = null) {
        ...
        $supplierDues = $this->db->query("
            ...
            (select (bill + cashReceived + material_bill) - (paid + returned)) as due
    ...
    public function productStock($productId) {
        ...
        if($stockCount != 0){
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->production_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity) 
