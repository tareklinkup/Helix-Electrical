				...
				<div class="col-xs-6">
					<table class="balancesheet-table">
						...
						<tbody>
							...
							<tr>
								<th>Material Stock Value</th>
								<td style="text-align:right;">{{material_stock_value | decimal}}</td>
							</tr>
			...
			async getStatements() {
				...
				.then(res => {
					if(res.data.success){
						...
						this.material_stock_value	= res.data.statements.material_stock_value ?? 0;
						...
						let totalAsset   	= 	parseFloat(this.assets) +
												...
												parseFloat(this.supplier_prev_due) +
												parseFloat(this.loss) +
												parseFloat(this.material_stock_value) +
												parseFloat(this.stock_value);
